...
export async function createdata(req, res) {
    const {
        ...
        data_atendimento,
        hora_fechou_robo,
        hora_ini_atendimento,  // Renomeado para 'hora_ini_atendimento'
        hora_fim_atendimento    // Renomeado para 'hora_fim_atendimento'
    } = req.body;

    try {
        // Verificar se as horas foram enviadas corretamente
        if (!hora_fechou_robo || !hora_ini_atendimento || !hora_fim_atendimento) {
            return res.status(400).json({ message: "Hora de fechamento do robô, início ou fim do atendimento não fornecida." });
        }
